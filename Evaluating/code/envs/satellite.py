...
class Satellite(DRVecTask):
    def __init__(self, config, rl_device, sim_device, graphics_device_id, headless):
        ...
        ###################################################
        self.log_trajectories =      config["env"].get('log_trajectories', False)
        ###################################################

        ###################################################
        self.explosion =        config["explosion"].get('enabled', False)
        self.explosion_time =   int(config["explosion"].get('explosion_time', 120) / self.dt)
        ###################################################
        
        ##################################################
        self.discretize_starting_pos = config["env"].get('discretize_starting_pos', False)
        ##################################################
